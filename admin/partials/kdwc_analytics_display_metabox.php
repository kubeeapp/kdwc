...
<?php if(!isset($_COOKIE['kdwc_hide_analytics_notice'])): ?>
    <div class="analytics-noticebox whileLoading purple-noticebox">
        <span class="closeX" style="border-color:#c0bc25;">X</span>
        <p>Kd-Wc does not count admin visits. Browse incognito to make tests.</p>
    </div>
<?php endif; ?>
    <h4 id="analytics-loading-notice" style="margin:20px 0 28px 0;font-weight:normal;text-align:center;"><?php _e('Loading stats...', 'kd-wc'); ?> </h4>
    <div id="analytics-container" pid="<?php echo $current_post_id ?>"></div>
    <p class="php-analytics-toggle-link"><span class="kdwc-turnme"><i class="fa fa-angle-down" aria-hidden="true"></i></span><?php _e('Set up a conversion', 'kd-wc'); ?></p>
    <div class="php-analytics-toggle-wrap">
        <p style="font-size:12.3px;line-height:1.3;margin-top:2px;">
            <?php _e('Paste the shortcode into your conversion page. Conversions will be assigned to the last version a visitor has seen.', 'kd-wc');?> <a target="_blank" href="https://www.kd-wc.com/help/documentation/analytics/?utm_source=Plugin&utm_medium=settings&utm_campaign=analyticsConversionOptionsLarnMore#anc_conversion-options"><?php _e('Advanced Options', 'kd-wc'); ?></a>
        </p>

        <span class="shortcode"><input type="text" onfocus="this.select();" readonly="readonly" value='[kdwc_conversion]' class="large-text code"></span>
    ...
    <script>
        ...
        function buildAnalyticsDisplay(res){
            ...
            container.innerHTML = '';
            data = JSON.parse(res);
            container.appendChild(createRow(['<?php _e('Version', 'kd-wc'); ?>','<?php _e('Views', 'kd-wc'); ?>','<?php _e('Conversions', 'kd-wc'); ?>','<?php _e('Conv. rate', 'kd-wc'); ?>']));
        ...
        function resetAllFields(){
            if(confirm('<?php _e("Are you sure you want to reset this trigger stats?", 'kd-wc'); ?>')){
        ...
        function resetVersionFields(version){
            if(confirm('<?php _e("Are you sure you want to reset this version stats?", 'kd-wc'); ?>')){
                ajaxPost({action:'kdwc_analytics_req',an_action:'resetFields',postid:pid,versionid:version}, function(){
                    refreshAnalyticsDisplay(pid);
                })
            }
        }
...
<?php else: ?>
    <h4 style="margin-bottom:8px;font-weight:normal;"><?php _e('Statistics will be available after publish', 'kd-wc'); ?></h4>
<?php endif; ?>