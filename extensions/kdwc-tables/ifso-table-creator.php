...
$local_user_table_name = $db_prefix . 'kdwc_local_user';
$daily_sessions_table_name = $db_prefix . 'kdwc_daily_sessions';

if($wpdb->get_var("SHOW TABLES LIKE 'kdwc_local_user'") || $wpdb->get_var("SHOW TABLES LIKE 'kdwc_daily_sessions'"))        //If tables with the old table names still exist, rename them to the new names
    $wpdb->query("RENAME TABLE kdwc_local_user TO {$local_user_table_name}, kdwc_daily_sessions TO {$daily_sessions_table_name}");
...
function jal_install() {
    ...
    $sql = "CREATE TABLE IF NOT EXISTS {$local_user_table_name} (
        ...
        //$sql = "UPDATE kdwc_local_user SET (`user_email`, `user_bank`, `user_sessions`, `alert_values`) VALUES ('$wp_user_email', '$geo_monthly_queries', '$geo_queries_used', '100 90 75 60') WHERE `id` =  1";
