(function( $ ) {
	...
	function activateFreezeTooltip() {
		var freezeTooltipClass = ".kdwc-freeze-overlay";
		var freezeTooltipStyleClass = "kdwc_freeze_tooltip_styling";
	...
	activeTooltip("kdwc_tooltip");
	activateFreezeTooltip();
	 
	$(document).ready(function () {
		/* KDWC first-use instructions */
		if ( $("#kdwc-modal-first-use").length ) {
			
			console.log("#kdwc-modal-first-use exists");
			$(".kdwc-first-use-images").modaal({
			    type: 'image'
			});
			$(".kdwc-first-use-images").first().modaal('open');
		}
		
		 // Enable Time/Day Schedule
		 $(".date-time-schedule").dayScheduleSelector(scheduleSettings);
		// Enable DateTimePicker
		$('.kdwcdatetimepicker').kdwcdatetimepicker();
		...
		$(document).on( 'click', '#reapeater-add', function(btnprs) {
			...
			var $newCountryAutocompleteInput = $('<input>').attr({
				type: 'text',
				class: 'countries-autocomplete kdwc-input-autocomplete',
			...
			var $newContinentAutocompleteInput = $('<input>').attr({
				type: 'text',
				class: 'continents-autocomplete kdwc-input-autocomplete',
			...
			var $newStateAutocompleteInput = $('<input>').attr({
				type: 'text',
				class: 'states-autocomplete kdwc-input-autocomplete',
			...
			jQuery.post(ajaxurl, data, function(response) {
				...
				// $('.post-type-kdwc_triggers #post').validator('update');
				 $(".date-time-schedule").dayScheduleSelector(scheduleSettings);
				...
				$('.datetimepickercustom-' + datetime_index).kdwcdatetimepicker();
				if (!isScrolled) {
					isScrolled = true;
					/*setTimeout(function() {
		...
		$(document).on( 'click', '.php-shortcode-toggle-link', function(e) {
			...
			e.target.querySelector('.kdwc-turnme').classList.toggle('kdwc-turnt-around')
		});
		...
		$('.post-type-kdwc_triggers #post').on('submit', function (e) {
		...
		$(document).on('change', '.kdwc-autocomplete-opener', function() {
			var $this = $(this);
			var effectRate = 250;
			// Handle already shown element
			var $currentShownElem = $('.kdwc-geo-selected');
			$currentShownElem.stop(true).slideUp(effectRate);
			$currentShownElem.removeClass('kdwc-geo-selected');
			// Handle new element
			var classNameOfElemToShow = $this.data("open");
			var $elemToShow = $("." + classNameOfElemToShow);
			$elemToShow.addClass('kdwc-geo-selected');
		...
		if(window.location.href.indexOf("post-new.php?post_type=kdwc_triggers") > -1) {
			$('a[href="'+window.location.href+'"]').closest('li').addClass('current');
		}
		
		/* Make Version Draggable & Droppable */
		// $("#kdwc-versions-container").DDSort({
		// 	target: 'li',
		// 	down: function(){
		// 		console.log("Mouse is down");
		// 	},
		// });
		$( "#kdwc-versions-container .kdwc-versions-sortable" ).sortable({
				handle: '.kdwc-btn-drag',
		...
		$( "#kdwc-versions-container .kdwc-versions-sortable" ).disableSelection();
		$(".advPlatformsCode, .kdwc-dynamic-link-code, .wp-editor-area").on("mouseenter", function(){
			$( "#kdwc-versions-container .kdwc-versions-sortable" ).enableSelection();
		});
		$(".advPlatformsCode, .kdwc-dynamic-link-code, .wp-editor-area").on("mouseleave", function(){
			$( "#kdwc-versions-container .kdwc-versions-sortable" ).disableSelection();
		});
		$(".kdwc-versions-sortable").keydown(function(e){
		...
		if($('.kdwc-autocomplete-container.select-city-container').length>0){
			showOnFocus($('.kdwc-autocomplete-container.select-city-container input'),$('.kdwc-form-group .geo-info-container'),$('.kdwc-form-group .setgeoinstructions .closeXGeo'),'geo_city_accuracy');
		}
		...
		if($('#kdwc-modal-caching-compat').length>0){
			showOnFocus(null,$('#kdwc-modal-caching-compat'),$('#kdwc-modal-caching-compat .buttons .neverAgain'),'caching_modal');
			$('#kdwc-modal-caching-compat .buttons .cls').on('click',function(){
				$(this).closest('#kdwc-modal-caching-compat').hide();
			});
		}
		...
		if($('#kdwc_send_test_email').length>0){
			$('#kdwc_send_test_email').on('click',function(){ajaxPost({action:'send_test_mail'},function(){alert('A testing email was sent successfully. Please check your spam folder if you do not see it in your inbox.')},function(){alert('Something went wrong! Please check your internet connection and try again!')})});
		}
	...
	var overlayDivHTML = '<div class="kdwc-tm-overlay"><span class="text">Testing Mode <br/><br/> <span class="cancel-freezemode"> Another version is forced to be displayed </span> </span></div>';
	var overlayFreezeHTML = '<div class="kdwc-freeze-overlay kdwc_tooltip"><span class="text">Version is inactive</span> </div>';
	var selectedTestingMode = false;
	function disableTestingMode($elem, $repeaterParent, isDefaultRepeater) {
		// before appending, removing all the 'kdwc-tm-overlay' present
		// due to prior appending
		$(".kdwc-tm-overlay").remove();
		$("#tm-input").attr("value", "");
	}
	...
	$(document).on("click", ".kdwc-tm", function(e) {		
		var $elem = $(this);
		...
		if (selectedTestingMode) {
			...
			$(".kdwc-tm").removeClass("circle-active");
	...
	$(document).on("click", ".kdwc-freezemode", function(e) {		
		var $elem = $(this);
		...
		if (isActive) {
			// Handle deactive
			$ancParent.find(".kdwc-freeze-overlay").remove();
	...
	$(document).on("click", ".groups-expander", function() {
		var $this = $(this);
		var $recSelectionContainer = $this.closest('.kdwc-form-group').find(".groups-selection");
		$recSelectionContainer.stop(true).toggle();
	...
	$(document).on("click", ".kdwc-license-tabs-header .kdwc-tab", function() {
...
function showOnFocus(toFocus,toShow,closingX,notif_id){
	...
	if(notif_id===false || !getCookie('kdwc_hide_'+notif_id)){
		if(typeof(toFocus)!='undefined' && toFocus!==null) {
            ...
            toFocus.on('focus', function (e) {
                ...
                if (toShow.length > 1) jQuery(e.target).closest('.kdwc-form-group').find(toShow.selector).removeClass('nodisplay');
		...
		if(typeof(closingX!='undefined')){
			closingX.on('click',function(f){
				toShow.addClass('nodisplay');
				if(notif_id!==false){
					createCookie('kdwc_hide_'+notif_id,1,365);
				}
			})
		}
	}
...
function resetAllAnalyticsDataAction(){
	if(confirm('Are you sure you want to reset all of the analytics data accumulated by kd-wc? This data cannot be recovered')){
		ajaxPost({action:'kdwc_analytics_req',an_action:'resetAllAnalytics',postid:0}, function(){
			alert('All of the kd-wc analytics data has been deleted!');
