...
wp.blocks.registerBlockType('kdwc/kdwc-block', {
    ...
    edit:  window.wp.data.withSelect( function( select ) {
            return {
                posts: select( 'core' ).getEntityRecords( 'postType', 'kdwc_triggers', {per_page:-1} )
            };
        } )( function( props ) {
            ...
            if ( props.posts.length === 0 ) {
                ...
                return el('div',{className:'kdwc-block-wrapper components-placeholder kdwc-block-error ' + props.className},
                         [el('div',{className: 'components-placeholder__label'},
                            [iconEl,el('span',{className:'components-placeholder__instructions'},'Dynamic Content')])
                             ,el('span',{className:'components-placeholder__instructions'},'Select a dynamic trigger from the list'),el('span',{className:"errMsg components-placeholder__instructions"},[el('span',{className:'dashicons dashicons-info'},''),"You haven\'t set up any dynamic triggers"]),el('button',{onClick:function(){window.open(kdwc_base_url+"/wp-admin/post-new.php?post_type=kdwc_triggers","_blank")},className:'is-secondary components-button is-button is-large is-default'},"Create a new trigger")]);
            }
            ...
            var wrap = el('div',{className:'kdwc-block-wrapper components-placeholder ' + props.className},
                [el('div',{className: 'components-placeholder__label'},iconEl,el('span',{className:'components-placeholder__instructions'},'Dynamic Content')),el('span',{className:'components-placeholder__instructions'},'Select a dynamic trigger from the list'),sel,
                    el('div',{className:'kdwc-button-wrap'},[el('button',{className: 'is-secondary components-button is-button is-large is-default', onClick:function(){window.open(kdwc_base_url + '?post_type=kdwc_triggers&p=' + props.attributes.selected,'_blank')}},'View trigger'),
                    el('button',{className: 'is-secondary components-button is-button is-large is-default',onClick:function(){window.open(kdwc_base_url + '/wp-admin/post.php?action=edit&post=' + props.attributes.selected,'_blank')}},'Edit trigger')])
                ]);
