...
namespace KDWC\Services\GeoLicenseService;
...
class GeoLicenseService {
	...
	private $std_kdwc_array = array(9134, 5965, 2473, 9136, 9029, 35211, 35215, 35418);
	...
	private function query_kdwc_api($edd_action, $license, $item_id) {
	...
	private function edd_api_get_error_message($license_data) {
		...
		if ( false === $license_data->success ) {
			...
			switch( $license_data->error ) {
				...
				case 'expired' :
					$message = sprintf(
						__( 'Your license key has expired. !!!LINKSTART!!!https://www.kd-wc.com/plans/geolocation-plans?utm_source=Plugin&utm_medium=direct&utm_campaign=licenseExpired&utm_term=LicensePage&utm_content=a!!!LINKEND!!!!!!LINKTEXT!!!Click here to get a new license!!!LINKTEXTEND!!!' ),
				...
				case 'invalid_item_id':
					...
					$message = __( 'The license key is invalid for this version of the plugin. Make sure you have updated the plugin. If the problem persists, please contact us at support@kd-wc.com.' );
					break;
				case 'no_activations_left':
					$message = __( 'This license key is currently active in another domain. !!!LINKSTART!!!https://www.kd-wc.com/plans/geolocation-plans?kdwc=pro&utm_source=Plugin&utm_medium=LicenseErrors&utm_campaign=LicenseAlreadyActive!!!LINKEND!!!!!!LINKTEXT!!!Click here to get a new license!!!LINKTEXTEND!!!' );

					break;
				case 'domain_already_has_key':
					$message = __( 'A free trial license has already been used for this domain. !!!LINKSTART!!!https://www.kd-wc.com/plans/geolocation-plans?kdwc=pro&utm_source=Plugin&utm_medium=TrialEnded&utm_campaign=LicensePage!!!LINKEND!!!!!!LINKTEXT!!!Click here to get a pro license.!!!LINKTEXTEND!!!' );
					break;
				default :
					break;
			}
	...
	private function edd_api_activate_item($license, $item_id) {
		return $this->query_kdwc_api('activate_license', $license, $item_id);
	}

	public function deactivate_license_request($license, $item_id) {
		return $this->query_kdwc_api('deactivate_license', $license, $item_id);
	}
	...
	private function try_to_activate_license($license, $item_id) {
		...
		foreach ($this->plans as $key => $plan_id) {
			if ($plan_id != $item_id) {
				...
				if ( !$this->is_item_id_invalid_or_mismatch($license_data) ) {
					update_option( 'edd_kdwc_geo_license_item_id', $plan_id );
					return $license_data;
				}
			}
		}
	...
	private function edd_kdwc_if_license_valid_from_license_data($license_data) {
	...
	private function edd_kdwc_is_license_inactive($license_data) {
		// print_r($license_data);
	 ...
	 */
	public function edd_kdwc_activate_geo_license() {
		// listen for our activate button to be clicked
		if( isset( $_POST['edd_kdwc_geo_license_activate'] ) ) {
			// run a quick security check
		 	if( ! check_admin_referer( 'edd_kdwc_nonce', 'edd_kdwc_nonce' ) )
				return; // get out if we didn't click the Activate button
			// retrieve the license from the database
			$db_license = trim( get_option('edd_kdwc_geo_license_key') );
			$license = !empty($_POST['for_geo_deactivation']) && !$this->edd_api_get_error_message($_POST['for_geo_deactivation']) && substr(trim( $_POST["edd_kdwc_geo_license_key"] ), -5) ==  substr(trim( $_POST['for_geo_deactivation'] ), -5) ? $_POST['for_geo_deactivation'] : trim( $_POST["edd_kdwc_geo_license_key"] );
            $old_license_key = get_option('edd_kdwc_geo_license_key');
			if ($db_license != $license)
				delete_option('edd_kdwc_geo_license_status');
			// save the license in the database
			update_option('edd_kdwc_geo_license_key', $license);
			...
			if ( ! empty( $message ) ) {
				...
				$redirect = add_query_arg( array( 'sl_activation' => 'false', 'message' => urlencode( $message ),
					'method' => 'license','wrongLicenseGoto'=>$wrongLicenseGoto ), $base_url . '#kdwc-info-tab-geo' );//redirect back to the geo page's geolocation license tab
				wp_redirect( $redirect );
				exit();
			}
			// die($this->plans[$last_plan_indx]);
			// update_option('edd_kdwc_geo_license_item_id', $this->primary_product_id);
			// $license_data->license will be either "valid" or "invalid"
			update_option( 'edd_kdwc_geo_license_expires', $license_data->expires );
			update_option( 'edd_kdwc_geo_license_status', $license_data->license );
			update_option( 'edd_kdwc_geo_license_item_name', $license_data->item_name );
			update_option( 'edd_kdwc_had_geo_license', true );
			update_option( 'edd_kdwc_has_lifetime_geo_license', ($license_data->expires == 'lifetime') );
			delete_option( 'edd_kdwc_user_deactivated_geo_license' );
	...
	public function edd_kdwc_deactivate_geo_license() {
		// listen for our activate button to be clicked
		if( isset( $_POST['edd_kdwc_geo_license_deactivate'] ) ) {
			// run a quick security check
		 	if( ! check_admin_referer( 'edd_kdwc_nonce', 'edd_kdwc_nonce' ) )
				return; // get out if we didn't click the Activate button
			// retrieve the license from the database
			//$license = trim( get_option( 'edd_kdwc_geo_license_key' ) );
			...
			if ($license_data->success) {
				...
				if( $license_data->license == 'deactivated' ) {
					...
					delete_option( 'edd_kdwc_geo_license_status' );
					delete_option( 'edd_kdwc_geo_license_item_id' );
					delete_option( 'edd_kdwc_has_lifetime_geo_license' );
					delete_transient( 'kdwc_transient_geo_license_validation' );
					//if ( $license_data->expires == 'lifetime' ) {

						update_option( 'edd_kdwc_user_deactivated_geo_license', true );
					//}
				}
				$redirect = add_query_arg( array( 'method' => 'license' ), $base_url );
				wp_redirect( $redirect );
            ...
            if(isset($license_data->expires) && $license_data->expires < time()){  //Allow license deactivation if the key has expired
                delete_option( 'edd_kdwc_geo_license_status' );      //(other cases where deactivation has to happen despite an error?)
                delete_option( 'edd_kdwc_geo_license_item_id' );
                delete_option( 'edd_kdwc_has_lifetime_geo_license' );
                delete_transient( 'kdwc_transient_geo_license_validation' );
	 ...
	 *	KDWC Edd Updater
	 */
	public function edd_sl_kdwc_plugin_updater_geo() {
		if ( get_option( 'edd_kdwc_has_lifetime_geo_license' ) == 1 )
			return;
	...
	private function handle_invalid_license($reason) {
		update_option( 'edd_kdwc_geo_license_num_of_checks', 0 );
		update_option('edd_kdwc_geo_license_deactivation_reason', $reason);

		delete_option( 'edd_kdwc_geo_license_status' );
		delete_option( 'edd_kdwc_geo_license_item_id' );
		// set inetrval as invalid license
		set_transient( 'kdwc_transient_geo_license_validation',
					   true,
					   $this->interval_invalid_license_check );
	}

	/* Responsible to check if the license is still valid */
	public function edd_kdwc_is_geo_license_valid() {

		if ( !get_transient( 'kdwc_transient_geo_license_validation' ) ) {
			if ( get_option( 'edd_kdwc_has_lifetime_geo_license' ) == 1 ) { //לבדוק אם זו הסיבה
				set_transient( 'kdwc_transient_geo_license_validation',
							   true,
							   60 * 60 * 24 * 7 );
				return;
			...
			$item_id = get_option( 'edd_kdwc_geo_license_item_id' ); // $this->primary_product_id;
			$is_license_valid = (get_option( 'edd_kdwc_geo_license_status' ) === 'valid');
			// Validation
			...
			// send request to KDWC server to check for license validy
			...
			if ( $license_data &&
				...
				$num_of_checks = get_option( 'edd_kdwc_geo_license_num_of_checks' );
				if ( $num_of_checks == false ) { // first time check
					update_option( 'edd_kdwc_geo_license_num_of_checks', 1 );
					// set inetrval as valid license
					set_transient( 'kdwc_transient_geo_license_validation',
								   true,
								   $this->interval_valid_license_check );
					...
					update_option( 'edd_kdwc_geo_license_num_of_checks', ($num_of_checks+1) );
					// set inetrval as valid license
					set_transient( 'kdwc_transient_geo_license_validation',
								   true,
								   $this->interval_valid_license_check );
	...
	public function get_license() {
		return get_option( 'edd_kdwc_geo_license_key' );
	}
}
