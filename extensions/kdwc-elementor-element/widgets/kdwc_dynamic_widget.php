...
class IFSO_Dynamic_Widget extends Widget_Base {
    ...
    public function get_keywords() {
        return [ 'dynamic', 'content', 'dynamic content', 'personalization', 'kd-wc', 'kdwc' ];
    }

    public function get_script_depends() {
        return [ 'kdwc-editor-js' ];
    }
    ...
    protected function _register_controls() {
        ...
        if ( ! empty( $trigger_list ) ) {
            ...
            $new_trigger_url = admin_url( 'post-new.php?post_type=kdwc_triggers' );
            $this->add_control(
                'add_a_trigger',
                [
        ...
        foreach ( $trigger_list as $key => $value ) {
            ...
            if ( $trigger_versions ) {
                ...
                $html = "<div class='kdwc-trigger-buttons'>
                ...
                $analytics_html = $this->get_trigger_analytics( $key );
                $this->add_control(
    ...
    protected function render() {
        ...
        $default       = get_post_meta( $trigger_id, 'kdwc_trigger_default', true );
        ...
        if ( Plugin::$instance->editor->is_edit_mode() ) {
            ...
            echo TriggersService\TriggersService::get_instance()->handle( [ 'id' => $trigger_id ] );
        }
    ...
    protected function get_trigger_versions( $trigger ) {
        $version_names = [];
        $default       = get_post_meta( $trigger, 'kdwc_trigger_default', false );
        $data_versions = get_post_meta( $trigger, 'kdwc_trigger_version', false );
    ...
    protected function get_trigger_analytics( $trigger_id ) {
        if ( get_post_status( $trigger_id ) == 'publish' ) {
            return "<h4 class='analytics-loading-notice' id='analytics-loading-notice-$trigger_id'
		                    style='margin-bottom:8px;font-weight:normal;text-align:center;'>" . __( 'Loading stats...', 'kd-wc' ) . "</h4>
		...
		constructAnalyticsUi();
            ...
            return "<h4 style='margin-bottom:8px;font-weight:normal;'>" . __( 'Statistics will be available after publish', 'kd-wc' ) . "</h4>";
        }
    }
    ...
    protected function get_overview_section( $trigger_id ) {
        ...
        $kdwc_trigger_rules = get_post_meta( $trigger_id, 'kdwc_trigger_rules', true );

        $html = '';
        if ( ! empty( $kdwc_trigger_rules ) ) {
            $all_triggers = json_decode( $kdwc_trigger_rules, true );
