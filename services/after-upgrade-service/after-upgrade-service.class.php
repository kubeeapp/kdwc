 ...
 *
 **/
namespace KDWC\Services\AfterUpgradeService;
...
class AfterUpgradeService {
    ...
    public static function handle(){
        if(self::isUpdated()){
            try{
                ...
                update_option('kdwc_wp_version',self::$code_version);
    ...
    private static function reactivate_licenses(){
        ...
        $geo_item_id = get_option('edd_kdwc_geo_license_item_id');
    ...
    private static function reset_metabox_order(){
        global $wpdb;
        $wpdb->update($wpdb->prefix.'usermeta',['meta_key'=>''],['meta_key'=>'meta-box-order_kdwc_triggers']);
    }
    ...
    private static function create_license_renew_columns_if_not_exist(){
        ...
        if(empty($checkrow)){
            //$wpdb->query("ALTER TABLE `kdwc_local_user` ADD `pro_renewal_date` DATE NULL DEFAULT NULL AFTER `alert_values`, ADD `geo_renewal_date` DATE NULL DEFAULT NULL AFTER `Pro_renewal_date`;");
