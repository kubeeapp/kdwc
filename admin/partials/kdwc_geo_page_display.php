<?php
	...
	$license = get_option( 'edd_kdwc_geo_license_key' );
	$status  = get_option( 'edd_kdwc_geo_license_status' );
	$expires = get_option( 'edd_kdwc_geo_license_expires' );
...
function get_notification_data() {
    ...
    $table = $wpdb->prefix . 'kdwc_local_user';
	...
	$table = $wpdb->prefix . 'kdwc_local_user';
    $daily_sessions_table_name = $wpdb->prefix . 'kdwc_daily_sessions';
   ...
   $noLicenseMessageBox = '<div class="no_license_message">'. __("Enter a Geolocation License to gain extra sessions. ", 'kd-wc') . '<a href="https://www.kd-wc.com/plans/geolocation-plans/?kdwc=geocredits" target="_blank">'. __("Click here to get a Geolocation license key", 'kd-wc') . '</a>.</div>';
?>
<html> 
<head>
...
<div class="wrap">
	<h2>
		<?php
		    _e('Kd-Wc Dynamic Content | Geolocation');
		?>
	</h2>
	<div class="kdwc-license-wrapper">
			<ul class="kdwc-license-tabs-header">
				<li class="kdwc-tab <?php echo $licenseTabHeaderExtraClasses; ?>" 					data-tab="kdwc-info-tab-geo">		<?php _e('Info', 'kd-wc');?>							</li>

				<li class="kdwc-tab <?php echo $infoTabHeaderExtraClasses ?>"
					data-tab="license-tab-wrapper">
					 <?php _e('Usage History', 'kd-wc'); ?>
				</li>
				<li class="kdwc-tab <?php echo $infoTabHeaderExtraClasses; ?>" 					data-tab="kdwc-info-tab-wrapper">			 <?php _e('Notifications', 'kd-wc'); ?>
							</li>

			</ul>

			<div class="kdwc-license-tabs-wrapper">

	<!-- Notifications tab contents: -->

				<div class="kdwc-info-tab-wrapper"
					 style="<?php echo $infoTabExtraStyles; ?>">

					<div class="geolocation-info-wrapper">	

<form method="post" action="" class="license-form">
<?php settings_fields('edd_kdwc_license'); ?>
<table class="form-table license-tbl">
	<tbody>
		<tr valign="top">
			...
			<span class="notification-line">  <i><?php _e('Check this box if you would like to be notified regarding your quota threshold', 'kd-wc'); ?></i> </span>
<!-- <i>    Check this box if you would like to be notified regarding your quota threshold</i> -->
			</td>
		</tr>
		<tr valign="top">
			<th class="licenseTable" scope="row" valign="top">
				<?php _e('Quota Threshold', 'kd-wc'); ?>
			</th>
			<td>
			<input type="checkbox" class="group1" id="1st-box" name="alert-checkbox-value-1" value="75" <?php echo in_array('75', $form_alert_values) ? 'checked' : '';?> <?php echo !in_array('100', $form_alert_values) ? 'disabled' : '';?> > <span class="notification-line">  <span class="percentage">75%</span> &nbsp 	   <i><?php _e('Receive an Email alert when quota reaches 75%', 'kd-wc'); ?></i> </span><br><br>
			<input type="checkbox" class="group1" id="2nd-box" name="alert-checkbox-value-2" value="95" <?php echo in_array('95', $form_alert_values) ? 'checked' : '';?> <?php echo !in_array('100', $form_alert_values) ? 'disabled' : '';?>>  <span class="notification-line">  <span class="percentage">95%</span>&nbsp&nbsp  <i><?php _e('Receive an Email alert when quota reaches 90%', 'kd-wc'); ?></i> </span><br><br>
			<input type="checkbox" class="group1" id="3rd-box" name="alert-checkbox-value-3" value="100" <?php echo in_array('100', $form_alert_values) ? 'checked' : '';?> <?php echo !in_array('100', $form_alert_values) ? 'disabled' : '';?>>  <span class="notification-line">  <span class="percentage">100%</span>&nbsp&nbsp  <i><?php _e('Receive an Email alert when quota reaches 100%', 'kd-wc'); ?></i></span><br>
			</td>
		</tr>
		...
		<tr valign="top">
			<th></th>
			<td style="padding-top:0;"><?php _e('Make sure emails do not get to spam.','kd-wc') ?> <a id="kdwc_send_test_email" href="#"><?php _e('Send a test email.','kd-wc') ?> </a></td>
		</tr>
	</tbody>
</table>
<br>
<input type="submit" class="button-primary" name="update_notifications" value=<?php _e('Save', 'kd-wc')?>>
...
<!--end of Notifications div  -->
	...
	<!-- Usage History tab contents: -->
				...
				<tr>
 				 <th><?php _e('Date','kd-wc');?></th>
 				 <th><?php _e('Geolocation sessions','kd-wc');?></th>
				</tr>
				</table>
				</div>
				...
				<table id="customers">
					<?php
						$results = $wpdb->get_results( "SELECT * FROM {$daily_sessions_table_name} ORDER BY id DESC");
										...
										if (count($results) < 1) {
											_e('<td><p>You haven`t used the geolocation condition</p><p>Usage statistics will be automatically recorded here as soon as a trigger with a geolocation condition is realized for the first time.</p></td></tr>','kd-wc');
										}
										?>
				</table>
			</div>
			...
			<div class="kdwc-info-tab-geo" style="<?php echo $licenseTabExtraStyles; ?>">
...
<?php
	...
	_e('<p class="kdwc-settings_paragraph">Geolocation is limited to 250 monthly <span title="A session is defined as beginning when a visitor first visits a page with a geolocation trigger and ends when a visitor closes the browser, the user ip is changed, or after 25 minutes of inactivity." class="tm-tip kdwc_tooltip line-tooltip">sessions</span>  with the free version and 1,000 monthly sessions for the duration of one year with the pro version.  <a class="buy-more-credits-link" href=" https://www.kd-wc.com/plans/geolocation-plans/?utm_source=Plugin&utm_medium=message&utm_campaign=geolocation&utm_term=info-top&utm_content=a" target="_blank">Click here</a> for additional options if your website handles a larger amount.</p>','kd-wc');
	?>
<div><?php _e('Sessions used this month:', 'kd-wc'); ?> <span style="font-weight:bold;"><?php echo $geo_queries_left . '/' . number_format($geo_int_monthly_queries); ?></span> <a href="https://www.kd-wc.com/plans/geolocation-plans/?utm_source=Plugin&utm_medium=message&utm_campaign=geolocation&utm_term=info-tab&utm_content=a" target="_blank">Upgrade</a></div>
<?php if (is_pro_license_exist($geoData) && is_plusgeo_license_exist($geoData)): ?>
	<div>&nbsp;&nbsp;&nbsp;&nbsp;<?php _e('Pro:', 'kd-wc'); ?> <span style="font-weight:bold;"><?php echo '(' . $separateRealizations['pro'] . '/' . $pro_license_bank . ')'; ?></span></div>
	<div>&nbsp;&nbsp;&nbsp;&nbsp;<?php _e('Geo:', 'kd-wc'); ?> <span style="font-weight:bold;"><?php echo '(' . $separateRealizations['geo'] . '/' . $geo_license_bank . ')'; ?></span></div>
<?php endif; ?>
...
<?php endif; ?> -->
				<div class="geo-license-section">
					<h1 class="kdwc-info-title" style=""><?php _e('Your subscription', 'kd-wc'); ?></h1>
					<div class="kdwc-info-content" style="font-size: 13px; line-height: 1.7;max-width: 1020px;">

						<div class="kdwc-info-content" >
							<span class="kdwc-content-head"><?php _e('Subscription:', 'kd-wc'); ?></span>
							<span class="kdwc-content-body"> <?php echo (($pro_license_type && $pro_license_type!='free' || $pro_license_type=='free' && !$geo_license_type ) ? ucwords($pro_license_type) . " ({$pro_license_bank} sessions)" : '') . (($geo_license_type && $pro_license_type && $pro_license_type!='free') ? ' + ' : '') . (($geo_license_type) ? ucwords($geo_license_type) . " ({$geo_license_bank} sessions)" : '');?> <a href="https://www.kd-wc.com/plans/geolocation-plans/?utm_source=Plugin&utm_medium=message&utm_campaign=geolocation&utm_term=info-tab&utm_content=a" target="_blank"><?php _e('Upgrade','kd-wc'); ?></a></span>
						</div>

						<div class="kdwc-info-content">
							<span class="kdwc-content-head"><?php _e('Queries left this month:', 'kd-wc'); ?></span>
							<span class="kdwc-content-body"><?php echo number_format($geo_int_monthly_queries - $geo_queries_left_send); ?>
</span>
						</div>
						...
						<!--	<div class="kdwc-info-content">
								<span class="kdwc-content-head">Product license purchase date:</span>
								<span class="kdwc-content-body"><?php// echo get_date_i18n($geo_pro_purchase_date); ?>
</span>
							</div>-->
			...
			<!--				<div class="kdwc-info-content">
								<span class="kdwc-content-head">Geolocation license purchase date:</span>
								<span class="kdwc-content-body">
		<?php// echo get_date_i18n(get_plusgeo_purchase_date($geoData)); ?>
									<span>
							</div>-->

							<div class="kdwc-info-content">
								<span class="kdwc-content-head"><?php _e('Geolocation license renewal date:', 'kd-wc'); ?></span>
								<span class="kdwc-content-body">
		<?php echo get_date_i18n(get_plusgeo_renewal_date($geoData)); ?>
						...
						<?php elseif (is_pro_license_exist($geoData)): ?>
							...
							<div class="kdwc-info-content">
								<span class="kdwc-content-head"><?php _e('Pro license renewal date:', 'kd-wc'); ?></span>
								<span class="kdwc-content-body"><?php echo get_date_i18n($geo_pro_renewal_date); ?>
</span>
							</div>
...
<h1 style="margin-top:20px;"><?php _e('Geolocation License', 'kd-wc'); ?></h1>
<p><?php _e('Enter your geolocation license key to upgrade the sessions amount.', 'kd-wc'); ?> <a href="https://www.kd-wc.com/plans/geolocation-plans?kdwc=geocredits" target="_blank"><?php _e('Get a license.', 'kd-wc'); ?></a></p>
<table class="form-table license-tbl">
	<tbody>
		<tr valign="top">
			<th class="licenseTable" scope="row" valign="top">
			...
			<td>
				<input id="edd_kdwc_geo_license_key" <?php echo ( is_license_valid( $status ) ) ? "readonly":""; ?>
				name="edd_kdwc_geo_license_key" type="text" class="regular-text" placeholder=<?php echo ($license) ? $dummy_license : '&nbsp;';?>>
				...
				<?php
					if ( $this->edd_kdwc_is_in_activations_process() ) {
						...
						$error_message = $this->edd_kdwc_get_error_message();
				...
				?>
					...
					<label class="description" for="edd_kdwc_geo_license_key"><?php _e('Enter your license key', 'kd-wc'); ?></label>
					<?php else: ?>
						<label class="description active"><?php _e('Active', 'kd-wc'); ?></label>
					<?php endif;?>
		...
		<tr valign="top">
			...
			<td>
				<?php if( $status !== false && $status == 'valid' ) { ?>
					<?php wp_nonce_field( 'edd_kdwc_nonce', 'edd_kdwc_nonce' ); ?>
					<input type="submit" class="button-secondary" name="edd_kdwc_geo_license_deactivate" value="<?php _e('Deactivate License', 'kd-wc'); ?>"/>
				<?php } else {
					wp_nonce_field( 'edd_kdwc_nonce', 'edd_kdwc_nonce' ); ?>
					<input type="submit" class="button-secondary" name="edd_kdwc_geo_license_activate" value="<?php _e('Activate License', 'kd-wc'); ?>"/>
				<?php } ?>
			</td>
		</tr>
	</tbody>
...
<!-- License key expiratiaton date -->
<?php if ($status == 'valid' && $expires == 'lifetime') { ?>
	<div class="license_expires_message"><?php _e('Your Geolocation License is Activated.', 'kd-wc'); ?></span></div>
<?php } else if ( $status == 'valid' && $expires !== false ) { ?>
	<div class="license_expires_message"><?php _e('Your license key expires on ', 'kd-wc'); ?><span class="expire_date"><?php echo date_i18n( 'F j, Y', strtotime( $expires, current_time( 'timestamp' ) ) ); ?>.</span></div>
<?php } ?>


</div>
		</div> <!-- end of kdwc-settings-tabs-wrapper -->
	</div>
