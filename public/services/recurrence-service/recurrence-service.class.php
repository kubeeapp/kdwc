<?php
	namespace KDWC\PublicFace\Services\RecurrenceService;
	...
	class RecurrenceService {
		...
		private function _cookies_handle($rule_data) {
			...
			if ($recurrence_type == 'always' ||
				...
				if (isset($_COOKIE['kdwc_recurrence_data'])) {
					$kdwc_recurrence_data_json = stripslashes($_COOKIE['kdwc_recurrence_data']);
					$kdwc_recurrence_data = json_decode($kdwc_recurrence_data_json, true);
				}
				// Set the new trigger
				$kdwc_recurrence_data[$trigger_id] = array(
				...
				// set the cookie
				setcookie('kdwc_recurrence_data', 
						  json_encode($kdwc_recurrence_data, JSON_UNESCAPED_UNICODE), 
						  time() + (86400 * 356 * 3),
						  '/'); // 3 years (356 * 3) days. 86400 = 1 day in second.
                $_COOKIE['kdwc_recurrence_data'] = json_encode($kdwc_recurrence_data, JSON_UNESCAPED_UNICODE);

			} else if ($recurrence_type == 'all-session') {
				$kdwc_recurrence_session_data = array(
													'version_index' => $recurrence_version_index,
													'trigger_type' => $recurrence_trigger_type,
													'recurrence_type' => $recurrence_type
												);
				$kdwc_recurrence_session_data_json = json_encode($kdwc_recurrence_session_data, JSON_UNESCAPED_UNICODE);
				setcookie('kdwc_recurrence_session_' . $trigger_id, $kdwc_recurrence_session_data_json, 0, '/');
			}
		}
