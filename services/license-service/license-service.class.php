...
namespace KDWC\Services\LicenseService;
...
class LicenseService {
	...
	private function edd_api_get_error_message($license_data) {
		...
		if ( false === $license_data->success ) {
			...
			switch( $license_data->error ) {
				...
				case 'expired' :
					$message = sprintf(
						__( 'Your license key has expired. !!!LINKSTART!!!https://www.kd-wc.com/plans?utm_source=Plugin&utm_medium=direct&utm_campaign=licenseExpired&utm_term=LicensePage&utm_content=a!!!LINKEND!!!!!!LINKTEXT!!!Click here to get a new license!!!LINKTEXTEND!!!' ),
				...
				case 'no_activations_left':
					$message = __( 'This license key is currently active in another domain. !!!LINKSTART!!!http://www.kd-wc.com/plans?kdwc=pro&utm_source=Plugin&utm_medium=LicenseErrors&utm_campaign=LicenseAlreadyActive!!!LINKEND!!!!!!LINKTEXT!!!Click here to get a new license!!!LINKTEXTEND!!!' );
					
					break;
				case 'domain_already_has_key':
					$message = __( 'A free trial license has already been used for this domain. !!!LINKSTART!!!http://www.kd-wc.com/plans?kdwc=pro&utm_source=Plugin&utm_medium=TrialEnded&utm_campaign=LicensePage!!!LINKEND!!!!!!LINKTEXT!!!Click here to get a pro license.!!!LINKTEXTEND!!!' );
					break;
				default :
					break;
			}
	...
	private function try_to_activate_license($license, $item_id) {
		...
		foreach ($this->plans as $key => $plan_id) {
			if ($plan_id != $item_id) {
				...
				if ( !$this->is_item_id_invalid_or_mismatch($license_data) ) {
					update_option( 'edd_kdwc_license_item_id', $plan_id );
					return $license_data;
				}
			}
		}
	 ...
	 */
	public function edd_kdwc_activate_license() {
		// listen for our activate button to be clicked
		if( isset( $_POST['edd_kdwc_license_activate'] ) ) {
			// run a quick security check
		 	if( ! check_admin_referer( 'edd_kdwc_nonce', 'edd_kdwc_nonce' ) )
				return; // get out if we didn't click the Activate button
			// retrieve the license from the database
			$db_license = trim( get_option('edd_kdwc_license_key') );
			$license = !empty($_POST['for_deactivation']) && !$this->edd_api_get_error_message($_POST['for_deactivation']) && substr(trim( $_POST["edd_kdwc_license_key"] ), -5) ==  substr(trim( $_POST['for_deactivation'] ), -5) ? $_POST['for_deactivation'] : trim( $_POST["edd_kdwc_license_key"] );
            $old_license_key = get_option('edd_kdwc_license_key');
			if ($db_license != $license)
				delete_option('edd_kdwc_license_status');
			// save the license in the database
			update_option('edd_kdwc_license_key', $license);
			...
			$passed_license_id = get_option('edd_kdwc_license_item_id');
            $wrongLicenseGoto='false';
			if(empty($message) && in_array($passed_license_id, $this->geo_array)){
                $geo_page_link = urlencode(admin_url('admin.php?page=' . EDD_KDWC_PLUGIN_GEO_PAGE));
                $message .= __("You have entered a Geolocation License. To activate a Geolocation License please go to",'kd-wc')." <a href='{$geo_page_link}'> Kd-Wc > Geolocation</a>.";
                $wrongLicenseGoto = 'geo';
            }
			$base_url = admin_url( 'admin.php?page=' . EDD_KDWC_PLUGIN_LICENSE_PAGE);

			if ( ! empty( $message ) ) {
			    //If an error message was generated earlier, redirect back to the page, showing the relevant error messages
			    update_option('edd_kdwc_license_key',$old_license_key);
			...
			// update_option('edd_kdwc_license_item_id', $this->primary_product_id);
			// $license_data->license will be either "valid" or "invalid"
			update_option( 'edd_kdwc_license_expires', $license_data->expires );
			update_option( 'edd_kdwc_license_status', $license_data->license );
			update_option( 'edd_kdwc_license_item_name', $license_data->item_name );
			update_option( 'edd_kdwc_had_license', true );
			update_option( 'edd_kdwc_has_lifetime_license', ($license_data->expires == 'lifetime') );
			delete_option( 'edd_kdwc_user_deactivated_license' );
	...
	public function edd_kdwc_deactivate_license() {
		// listen for our activate button to be clicked
		if( isset( $_POST['edd_kdwc_license_deactivate'] ) ) {
			// run a quick security check
		 	if( ! check_admin_referer( 'edd_kdwc_nonce', 'edd_kdwc_nonce' ) )
				return; // get out if we didn't click the Activate button
			// retrieve the license from the database
			//$license = trim( get_option( 'edd_kdwc_license_key' ) );
			$license = trim( $_POST['for_deactivation'] );
			$item_id = get_option( 'edd_kdwc_license_item_id' );
			...
			if ($license_data->success) { //לבדוק מה מוחזר אם הרשיון לא קיים באד. לדוגמא, נמחק ידנית דרך אד.
				...
				if( $license_data->license == 'deactivated' ) {
					...
					delete_option( 'edd_kdwc_license_status' );
					delete_option( 'edd_kdwc_license_item_id' );
					delete_option( 'edd_kdwc_has_lifetime_license' );
					delete_transient( 'kdwc_transient_license_validation' );
					//if ( $license_data->expires == 'lifetime' ) {

						update_option( 'edd_kdwc_user_deactivated_license', true );
					//}
            ...
            if(isset($license_data->expires) && $license_data->expires < time()){  //Allow license deactivation if the key has expired
                delete_option( 'edd_kdwc_license_status' );              //(other cases where deactivation has to happen despite an error?)
                delete_option( 'edd_kdwc_license_item_id' );
                delete_option( 'edd_kdwc_has_lifetime_license' );
                delete_transient( 'kdwc_transient_license_validation' );
	 ...
	 *	KDWC Edd Updater 
	 */
	public function edd_sl_kdwc_plugin_updater() {
		if ( get_option( 'edd_kdwc_has_lifetime_license' ) == 1 )
			return;
		// retrieve our license key & item name from the DB
		$license = trim( get_option('edd_kdwc_license_key') );
		$item_id = get_option('edd_kdwc_license_item_id'); // $this->primary_product_id;
		if ( $license && $item_id ) {
			// setup the updater
	...
	private function handle_invalid_license($reason) {
		update_option( 'edd_kdwc_license_num_of_checks', 0 );
		update_option('edd_kdwc_license_deactivation_reason', $reason);
		
		delete_option( 'edd_kdwc_license_status' );
		delete_option( 'edd_kdwc_license_item_id' );
		// set inetrval as invalid license
		set_transient( 'kdwc_transient_license_validation',
					   true,
					   $this->interval_invalid_license_check );
	}

	/* Responsible to check if the license is still valid */
	public function edd_kdwc_is_license_valid() {
		
		if ( !get_transient( 'kdwc_transient_license_validation' ) ) {
			if ( get_option( 'edd_kdwc_has_lifetime_license' ) == 1 ) { //לבדוק אם זו הסיבה
				set_transient( 'kdwc_transient_license_validation',
							   true,
							   60 * 60 * 24 * 7 );
				return;
			...
			$item_id = get_option( 'edd_kdwc_license_item_id' ); // $this->primary_product_id;
			$is_license_valid = (get_option( 'edd_kdwc_license_status' ) === 'valid');
			// Validation
			...
			if ( $license_data && 
				...
				$num_of_checks = get_option( 'edd_kdwc_license_num_of_checks' );
				if ( $num_of_checks == false ) { // first time check
					update_option( 'edd_kdwc_license_num_of_checks', 1 );
					// set inetrval as valid license
					set_transient( 'kdwc_transient_license_validation',
								   true,
								   $this->interval_valid_license_check );
					...
					update_option( 'edd_kdwc_license_num_of_checks', ($num_of_checks+1) );
					// set inetrval as valid license
					set_transient( 'kdwc_transient_license_validation',
								   true,
								   $this->interval_valid_license_check );
	...
	public function get_license() {
		return get_option( 'edd_kdwc_license_key' );
	}
}
